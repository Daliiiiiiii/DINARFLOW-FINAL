...
router.get('/maintenance-mode', async (req, res) => {
    try {
        ...
        res.json({ isMaintenanceMode: settings.isMaintenanceMode });
    } catch (error) {
...
// Update maintenance mode status (superadmin only)
router.put('/maintenance-mode', auth, isSuperAdmin, async (req, res) => {
    try {
        ...
        console.error('Error updating maintenance mode status:', error);
        res.status(500).json({ error: 'Failed to update maintenance mode status' });
    }
});
