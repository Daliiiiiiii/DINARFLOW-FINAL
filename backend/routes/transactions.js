...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const transactions = await Transaction.find(filter)
      ...
      .populate('recipientId', 'firstName lastName') // Populate recipient details
      .sort({ createdAt: -1 })
      .lean(); // Return plain JavaScript objects

    // Map through transactions to add sender/recipient full names
    const transactionsWithFullNames = transactions.map(transaction => {
      const transactionObject = { ...transaction }; // Create a mutable copy
      ...
      if (transactionObject.senderId) {
        ...
        delete transactionObject.senderId; // Clean up populated object if not needed elsewhere
      }
      ...
      if (transactionObject.recipientId) {
        ...
        delete transactionObject.recipientId; // Clean up populated object if not needed elsewhere
      }

      // Also, for 'transfer' type with subtype 'receive' or 'send', update metadata if needed
      // This ensures that the frontend code that looks at metadata still works with the full name
