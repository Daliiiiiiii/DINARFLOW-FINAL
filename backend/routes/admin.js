...
router.get('/kyc-requests', [auth, adminAuth], async (req, res) => {
    try {
        ...
        if (dateRange) {
            ...
            if (startDate) {
                query['kyc.submittedAt'] = { $gte: startDate };
            }
        }
        ...
        const formattedRequests = requests.map(user => {
            ...
            if (user.kyc && Array.isArray(user.kyc.submissions) && user.kyc.submissions.length > 0) {
                ...
                if (user.kyc.currentSubmission >= 0) {
                    current = user.kyc.submissions[user.kyc.currentSubmission];
            ...
            return {
                ...
                status: current?.status || user.kyc?.status,
...
router.post('/kyc/:userId/approve', [auth, adminAuth], async (req, res) => {
    try {
        ...
        if (!user.kyc || user.kyc.currentSubmission === -1) {
            return res.status(400).json({ error: 'No active KYC submission found' });
        }
