...
export function AuthProvider({ children }) {
  ...
  async function login(email, password, rememberMe = false) {
    try {
      ...
      // Fetch the latest user profile to ensure KYC status is up-to-date
      let latestUser = data.user;
      try {
        const profileRes = await api.get('/api/auth/validate');
        if (profileRes.data && profileRes.data.user) {
          latestUser = profileRes.data.user;
        }
      } catch (e) {
        // fallback to login response if validate fails
      }
      ...
      const formattedUser = formatUserData({
        ...latestUser,
        profilePicture: storedProfilePicture || latestUser.profilePicture
      });
