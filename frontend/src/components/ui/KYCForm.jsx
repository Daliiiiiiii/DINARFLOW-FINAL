...
const KYCForm = ({ onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (countdown === 0) return; // Don't start timer if countdown is 0
    ...
    const timer = setInterval(() => {
      setCountdown(prevCountdown => {
        if (prevCountdown > 1) {
          ...
          clearInterval(timer);
          if (isSubmitted || showPendingAnimation) {
  ...
  }, [isSubmitted, showPendingAnimation, countdown]);
          ...
          {isSubmitted ? (
            ...
            >
              <div className="w-16 h-16 bg-orange-500/20 rounded-full flex items-center justify-center mx-auto mb-6">
                <Clock className="w-8 h-8 text-orange-400 animate-spin" />
              </div>
              <h3 className="text-xl font-semibold text-white mb-2">
                {t('kycForm.faceVerificationPending')}
              </h3>
              <p className="text-gray-400 mb-6">
                {t('kycForm.verificationInProgress')}
              </p>
              <div className={`${isDark ? 'bg-orange-500/10' : 'bg-orange-100'} rounded-lg p-4 mb-6`}>
                <div className="flex items-center gap-3">
