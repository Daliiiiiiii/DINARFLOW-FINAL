...
const Dashboard = () => {
  ...
  const statsCards = [
    {
      ...
      value: showKycOverlay ? mockStats.totalBalance.toFixed(2) : (walletBalance + totalBankBalance).toFixed(2),
      icon: Wallet
    },
    {
  ...
  useEffect(() => {
    ...
    // Calculate starting balance by reversing transactions after the period
    let startingBalance = currentUser?.walletBalance || 0;
    for (const t of allSorted) {
      const tDate = new Date(t.createdAt);
      if (tDate > now || tDate < startDate) continue;
      if (t.type === 'transfer') {
        if (t.subtype === 'send' && String(t.userId) === userId) startingBalance += Math.abs(t.amount);
        if (t.subtype === 'receive' && String(t.recipientId) === userId) startingBalance -= Math.abs(t.amount);
      }
      if (t.type === 'bank') {
        if (t.subtype === 'deposit' && String(t.userId) === userId) startingBalance -= Math.abs(t.amount);
        if (t.subtype === 'withdrawal' && String(t.userId) === userId) startingBalance += Math.abs(t.amount);
      }
    }
    ...
    // Calculate income (all money received by the user, regardless of currency or type)
    ...
    // Calculate expenses (money sent in TND only)
    ...
    // Generate chart data with correct running balance and all income/outcome (not just TND)
    const chartData = [];
    let chartBalance = (currentUser?.walletBalance || 0) + (currentUser?.bankBalance || 0);
    let day = new Date(startDate);
    while (day <= now) {
      ...
      for (const t of dayTxs) {
        // Income: transfer:receive to user, bank:deposit to user
        if (
          (t.type === 'transfer' && t.subtype === 'receive' && String(t.recipientId) === userId) ||
          (t.type === 'bank' && t.subtype === 'deposit' && String(t.userId) === userId)
        ) {
        ...
        // Outcome: transfer:send from user, bank:withdrawal from user
        if (
            ...
            <p className="text-3xl font-semibold">
              {showKycOverlay ? stat.value : (
                stat.title === t('dashboard.totalBalance') 
                  ? (walletBalance + totalBankBalance).toFixed(2)
                  : stat.value
              )}
            </p>
          </motion.div>
