...
const Transfer = () => {
  ...
  const [showBankLimits, setShowBankLimits] = useState(false);
  ...
  useEffect(() => {
    if (transferStatus?.type === 'success') {
      // Only update limits for user-to-user transfers
      // Never update limits for internal wallet-bank transfers
      if (transferType === 'user' && !direction) {
        fetchLimitsAndUsage();
      }
    }
  }, [transferStatus, fetchLimitsAndUsage, transferType, direction]);
          ...
          <div className="space-y-6">
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-4">
                  <h2 className="text-lg font-semibold">{t('transfer.limits')}</h2>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setShowBankLimits(false)}
                    ...
                    <button
                      onClick={() => setShowBankLimits(true)}
                      className={`px-3 py-1 rounded-lg text-sm transition-colors ${
                        showBankLimits
                          ? isDark
                            ? 'bg-green-600 text-white'
              ...
              <AnimatePresence mode="wait">
                <motion.div
                  key={showBankLimits ? 'bank' : 'wallet'}
                  initial={{ opacity: 0, x: showBankLimits ? 20 : -20 }}
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: showBankLimits ? -20 : 20 }}
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="font-medium">
                            {formatNumber(showBankLimits ? bankTransferLimits.daily : transferLimits.daily)} TND
                          </span>
                        </div>
                        <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full">
                          <div 
                            className={`h-2 rounded-full transition-all duration-300 ${
                              calculatePercentage(
                                showBankLimits ? limitsUsage.bank.daily : limitsUsage.transfer.daily,
                                showBankLimits ? bankTransferLimits.daily : transferLimits.daily
                              ) > 90 ? 'bg-red-500' : showBankLimits ? 'bg-green-600' : 'bg-blue-600'
                            }`} 
                            style={{ 
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                          {formatNumber(showBankLimits ? limitsUsage.bank.daily : limitsUsage.transfer.daily)} TND {t('transfer.used')} / {formatNumber(showBankLimits ? bankTransferLimits.daily : transferLimits.daily)} TND
                        </p>
                      </div>
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="font-medium">
                            {formatNumber(showBankLimits ? bankTransferLimits.weekly : transferLimits.weekly)} TND
                          </span>
                        </div>
                        <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full">
                          <div 
                            className={`h-2 rounded-full transition-all duration-300 ${
                              calculatePercentage(
                                showBankLimits ? limitsUsage.bank.weekly : limitsUsage.transfer.weekly,
                                showBankLimits ? bankTransferLimits.weekly : transferLimits.weekly
                              ) > 90 ? 'bg-red-500' : showBankLimits ? 'bg-green-600' : 'bg-blue-600'
                            }`} 
                            style={{ 
                              width: `${calculatePercentage(
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                          {formatNumber(showBankLimits ? limitsUsage.bank.weekly : limitsUsage.transfer.weekly)} TND {t('transfer.used')} / {formatNumber(showBankLimits ? bankTransferLimits.weekly : transferLimits.weekly)} TND
                        </p>
                      </div>
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="font-medium">
                            {formatNumber(showBankLimits ? bankTransferLimits.monthly : transferLimits.monthly)} TND
                          </span>
                        </div>
                        <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full">
                          <div 
                            className={`h-2 rounded-full transition-all duration-300 ${
                              calculatePercentage(
                                showBankLimits ? limitsUsage.bank.monthly : limitsUsage.transfer.monthly,
                                showBankLimits ? bankTransferLimits.monthly : transferLimits.monthly
                              ) > 90 ? 'bg-red-500' : showBankLimits ? 'bg-green-600' : 'bg-blue-600'
                            }`} 
                            style={{ 
                              width: `${calculatePercentage(
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                          {formatNumber(showBankLimits ? limitsUsage.bank.monthly : limitsUsage.transfer.monthly)} TND {t('transfer.used')} / {formatNumber(showBankLimits ? bankTransferLimits.monthly : transferLimits.monthly)} TND
                        </p>
                      </div>
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="font-medium">
                            {formatNumber(showBankLimits ? bankTransferLimits.perTransaction : transferLimits.perTransaction)} TND
                          </span>
                        </div>
                        <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full">
                          <div 
                            className={`h-2 rounded-full transition-all duration-300 ${
                              showBankLimits ? 'bg-green-600' : 'bg-blue-600'
                            }`} 
                            style={{ width: '100%' }} 
                          />
                        </div>
            ...
            >
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {transactions
                  .filter(transfer => 
                    // Exclude crypto and top-up transactions
                    transfer.type !== 'crypto' && 
                    transfer.type !== 'topup' && // Keep explicit topup type check
                    !(transfer.type === 'transfer' && transfer.subtype === 'topup') && // Keep existing check for transfer subtype
                    !(transfer.description && transfer.description.includes('Wallet top-up')) // Add check for description containing "Wallet top-up"
                  )
                  .slice(0, 10) // Show only last 10 transfers
                  .map((transfer, index) => (
                  <motion.div
                    key={index}
                    initial={{ opacity: 0, y: 20 }}
